...
DeferredLightingTest::DeferredLightingTest(QWidget *parent)
    : GenericGL(parent)
{
    xRot = 0;//4800;
    yRot = 0;//4800;
...
DeferredLightingTest::~DeferredLightingTest()
{
    makeCurrent();
    //glDeleteLists(object, 1);
}

QSize DeferredLightingTest::minimumSizeHint() const
{
    return QSize(50, 50);
}

QSize DeferredLightingTest::sizeHint() const
{
    return QSize(400, 400);
}

void DeferredLightingTest::initializeGL()
{
//	makeCurrent();
	m_Renderer.init();
	...
	m_Program = new Program( "glsl/testGbuf.vs.glsl", "glsl/testGbuf.fs.glsl" );
	...
	m_FSProgram = new Program( "glsl/testDefLit.vs.glsl", "glsl/testDefLit.fs.glsl" );
...
void DeferredLightingTest::paintGL()
{
...
//	makeCurrent();
    ...
    m_RenderTarget->getTexture( 1 )->use( 1, m_FSProgram );
...
void DeferredLightingTest::resetViewport()
{
    m_DefaultRenderTarget->resize( m_Width, m_Height );
}

void DeferredLightingTest::resizeGL(int width, int height)
{
	m_Width = width;
	m_Height = height;
	resetViewport();
...
void DeferredLightingTest::normalizeAngle(int *angle)
{
    while (*angle < 0)
        *angle += 360 * 16;
    while (*angle > 360 * 16)
