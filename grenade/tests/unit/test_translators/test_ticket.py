...
from grenade.translators.ticket import TicketTranslator
from probe.fixtures.mock_shotgun import MockShotgun

class TestTicketTranslator(object):
    """
    Nose unit test suite for Grenade TicketTranslator.
    ...
    def setup(self):
        ...
        self.translator = TicketTranslator(self.session)
    ...
    def test_translate(self):
        ...
        verify_translate(self.translator, 'sg_assigned_to', [{'id':1, 'type':'Group'}, {'id':2, 'type':'HumanUser'}], ['luke.cole', 'vfx'], ['anim'])
        verify_translate(self.translator, 'sg_shots', [{'id':4, 'type':'Shot'}], [{'Shot':[['id', 'is', 4]]}], [{'Shot':[['id', 'is', 5]]}])
